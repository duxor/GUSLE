<div id="pocetna" class="jumbotron text-center">
    <div class="container">
        <div class="slider-area">
            <h1>Веб портал гусле</h1>
            <p>Јер традиција нема цену. Добро нам дошли!</p>
        </div>
        <div class="baner-area scrol">
            <a href="#prodavnica" title="Веб продавница" class="baner">
                <img src="/img/templejt/baner-pocetna/baner-1.jpg">
                <h3><i class="glyphicon glyphicon-shopping-cart"></i> Веб продавница</h3>
                <p>најбољи ручни радови</p>
            </a>
        </div><br clear="all">
        <div class="slider-area">
            <div class="slider">
                <div id="slajder" class="carousel slide" data-ride="carousel">
                    <ol class="carousel-indicators">
                        <li data-target="#slajder" data-slide-to="0" class="active"></li>
                        <li data-target="#slajder" data-slide-to="1"></li>
                        <li data-target="#slajder" data-slide-to="2"></li>
                        <li data-target="#slajder" data-slide-to="3"></li>
                        <li data-target="#slajder" data-slide-to="4"></li>
                        <li data-target="#slajder" data-slide-to="5"></li>
                        <li data-target="#slajder" data-slide-to="6"></li>
                        <li data-target="#slajder" data-slide-to="7"></li>
                        <li data-target="#slajder" data-slide-to="8"></li>
                        <li data-target="#slajder" data-slide-to="9"></li>
                    </ol>
                    <div class="carousel-inner" role="listbox">
                        <div class="item active">
                            <img src="/img/templejt/slajder/dusan-silni.jpg" alt="Цар Душан Силни">
                            <div class="carousel-caption">
                                <h3>Славна српска историја</h3>
                                <p>Само дело које вреди постаће део предања!</p>
                            </div>
                        </div>
                        <div class="item">
                            <img src="/img/templejt/slajder/guslari-1.jpg" alt="Српска традиција">
                            <div class="carousel-caption">
                                <h3>Богата српска традиција</h3>
                                <p>Без гусала разговора нема</p>
                            </div>
                        </div>
                        <div class="item">
                            <img src="/img/templejt/slajder/srpska-majka.jpg" alt="Српска мајка">
                            <div class="carousel-caption">
                                <h3>Српска мајка</h3>
                                <p>Српкиња је мене мајка родила</p>
                            </div>
                        </div>
                        <div class="item">
                            <img src="/img/templejt/slajder/guslari-2.jpg" alt="Српска слога">
                            <div class="carousel-caption">
                                <h3>Српска слога</h3>
                                <p>Од кад нисмо за милога Бога, запевали један крај другога</p>
                            </div>
                        </div>
                        <div class="item">
                            <img src="/img/templejt/slajder/srpski-obicaji-1.jpg" alt="Српски обичаји">
                            <div class="carousel-caption">
                                <h3>Српски обичаји</h3>
                                <p></p>
                            </div>
                        </div>
                        <div class="item">
                            <img src="/img/templejt/slajder/1.jpg" alt="">
                            <div class="carousel-caption">
                                <h3>Богата српска традиција</h3>
                                <p></p>
                            </div>
                        </div>
                        <div class="item">
                            <img src="/img/templejt/slajder/6.jpg" alt="">
                            <div class="carousel-caption">
                                <h3>Богата српска традиција</h3>
                                <p></p>
                            </div>
                        </div>
                        <div class="item">
                            <img src="/img/templejt/slajder/7.jpg" alt="">
                            <div class="carousel-caption">
                                <h3>Богата српска традиција</h3>
                                <p></p>
                            </div>
                        </div>
                        <div class="item">
                            <img src="/img/templejt/slajder/11.jpg" alt="">
                            <div class="carousel-caption">
                                <h3>Богата српска традиција</h3>
                                <p></p>
                            </div>
                        </div>
                        <div class="item">
                            <img src="/img/templejt/slajder/13.jpg" alt="">
                            <div class="carousel-caption">
                                <h3>Богата српска традиција</h3>
                                <p></p>
                            </div>
                        </div>
                    </div>
                    <a class="left carousel-control" href="#slajder" role="button" data-slide="prev">
                        <span class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span>
                        <span class="sr-only">Претходни</span>
                    </a>
                    <a class="right carousel-control" href="#slajder" role="button" data-slide="next">
                        <span class="glyphicon glyphicon-chevron-right" aria-hidden="true"></span>
                        <span class="sr-only">Следећи</span>
                    </a>
                </div>
            </div>
        </div>
        <div class="baner-area scrol">
            <a href="#aktuelnosti" title="Актуелности" class="baner">
                <img src="/img/templejt/baner-pocetna/baner-2.jpg">
                <h3><i class="glyphicon glyphicon-calendar"></i> Актуелности</h3>
                <p>календар догађаја</p>
            </a>
            <a href="#istorijat" title="Историјат" class="baner">
                <img src="/img/templejt/baner-pocetna/baner-3.jpg">
                <h3><i class="glyphicon glyphicon-book"></i> Историјат</h3>
                <p>из историјских извора</p>
            </a>
            <a href="#istorijat" title="Историјат" class="baner">
                <img src="/img/templejt/baner-pocetna/baner-4.jpg">
                <h3><i class="icon-users-1"></i> Друштва</h3>
                <p>савези и друштва</p>
            </a>
        </div>
    </div>
</div>
<style>
    .slider-area{
        width: 70%;
        float:left;
        color: #131313;
    }
    .slider{
        margin-top: 15px;
        width: 100%;
    }
    .slider img{
        width:100%;
        height: 100%;
    }
    .baner-area{
        width: 27%;
        float:left;
    }
    .baner{
        width: 100%;
        margin: 15px 0 0 2%;
        float:left;
        color: #131313;
    }
    .baner h3{
        position: absolute;
        margin: -90px 0 0 20px;
        font-weight: bold;
    }
    .baner p{
        position: absolute;
        margin: -65px 0 0 55px;
        font-style: italic;
        font-size: 120%;
    }
</style>